...
in {
  ...
  config = lib.mkMerge [
    ...
    (lib.mkIf (!kmonadCfg.enable && !kanataCfg.enable) {
      ...
      #
      system.activationScripts.applications.text = pkgs.lib.mkForce (''
        ${pkgs.bash} '${karabinerFilesPath}/scripts/uninstall/deactivate_driver.sh'
        ${pkgs.bash} '${karabinerFilesPath}/scripts/uninstall/remove_files.sh'
        killall Karabiner-VirtualHIDDevice-Daemon
        if [ -d "${karabinerDriverKitExtDestPath}" ]; then
          echo "Removing Karabiner-VirtualHIDDevice-Manager.app..."
